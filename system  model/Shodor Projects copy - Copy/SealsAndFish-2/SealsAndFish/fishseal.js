...
/* Modifier: Phil List, Shodor, 2015 */
...
var N_SEALS = 15;
var N_FISH = 25;
var FISH_HUNGER_DURATION = 170;
var SEAL_HUNGER_DURATION = 180;
var theSeals = [];
var theFish = [];
...
//		if (yPos(theFish[agentCounter].loc) === N_ROWS - 1){
		
	//		theFish[agetCounter].hunger = FISH_HUNGER_DURATION;
...
tryEatF = () {
	...
	//while (theFish[agentCounter].hunger <= 50 && yPos(theFish[agentCounter].loc) !== N_ROWS - 1){
...
tryReproduce = function() {
	...
	for(agentCounter=0; agentCounter<theSeals.length; agentCounter++){
		...
		if(theSeals[agentCounter].reprodChance>=Math.random() && 
			...
			// baby seal added to the array of seals
			// baby seal added to the array of all creatures
		}
	}
}
...
move = function(){
	...
	for(agentCounter = 0; agentCounter < theSeals.length; agentCounter++){
		...
		if(strongestSound(theSeals[agentCounter].loc)>0){
			switch(strongestSound(theSeals[agentCounter].loc)){
				case 1:
 ...
 */
strongestSound = function(location){
	var dirs = [];
	var max = 0;
