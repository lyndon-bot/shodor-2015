...
fishImg.src = "img/fish.gif";
var sealImg = new Image();
sealImg.src = "img/seals.gif";
var fishTImg = new Image();
fishTImg.src = "img/fishT.gif";
var sealTImg = new Image();
sealTImg.src = "img/sealsT.gif";
var anyGreen = 10;
var Interval;
...
TfishObj = function(locationInGrid, howOld, howLoud){
	this.species		= 'Tfish';
	this.eatChance		= .9;
	this.reprodChance	= .05;
	this.hunger			= FISH_HUNGER_DURATION;
...
TsealObj = function(locationInGrid, howOld){
	this.species		= 'seal';
	this.eatChance		= .1;
	this.reprodChance	= .05;
	this.hunger			= SEAL_HUNGER_DURATION;
...
placeAgents = function() {
	...
	for(agentCounter = 0; agentCounter < N_FISH; agentCounter++){
		...
		theTFish[agentCounter] = new TfishObj(location, Math.floor(25*Math.random()), TAG_VOLUME);
		// mark in the location array that a fish is here
		theCells[location] = "Tfish";
		vol[location] = theTFish[agentCounter].vol;
	}
	...
	for(agentCounter = 0; agentCounter < N_SEALS; agentCounter++){
		...
		theCells[location] = "Tseal";
		// add a fish object into this array of seal objects, give it a random start age
		theTSeals[agentCounter] = new TsealObj(location, Math.floor(25*Math.random()));
	}
...
drawAnimals = function(){
	...
	for(i = 0; i < theTSeals.length; i++){
		Context.drawImage(TsealImg, xPos(theTSeals[i].loc)*WIDTH, yPos(theTSeals[i].loc)*HEIGHT);
	}
}
